...
void TessBaseAPI::GetTextPosition(int page_number, std::vector<DetectedText> &detectedTextList) {
	if (tesseract_ == NULL ||
		(!recognition_done_ && Recognize(NULL) < 0))
		return;
	int blob_count;
	...
	do {
		int left, top, right, bottom;
		if (it->BoundingBox(RIL_BLOCK, &left, &top, &right, &bottom)) {
			TextPosition textPosition(left, right, top, bottom);
			DetectedText detectedText;
			detectedText.position = textPosition;
			detectedText.text = it->GetUTF8Text(RIL_BLOCK);
			// Tesseract uses space for recognition failure. Fix to a reject
			// character, kTesseractReject so we don't create illegal box files.
			for (int i = 0; detectedText.text[i] != '\0'; ++i) {
				if (detectedText.text[i] == ' ')
					detectedText.text[i] = kTesseractReject;
			}
			
			detectedTextList.push_back(detectedText);
			output_length += strlen(result + output_length);
			// Just in case...
			if (output_length + kMaxBytesPerLine > total_length)
				break;

...
typedef struct TextPosition {
	...
	int right;
	int bottom;
	TextPosition(int initLeft, int initTop, int initRight, int initBottom) : left(initLeft), right(initRight), top(initTop), bottom(initBottom) { };
	TextPosition() : left(0), right(0), top(0), bottom(0) {};
};
typedef struct DetectedText {
	char* text = "";
...
class TESS_API TessBaseAPI {
  ...
  */
  ...
  void GetTextPosition(int page_number, std::vector<DetectedText> &detectedTextList);
  /**
   * The recognized text is returned as a char* which is coded
